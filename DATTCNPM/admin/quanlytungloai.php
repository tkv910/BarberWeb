        ...
        <div id="page-content-wrapper">
            <nav class="navbar navbar-expand-lg navbar-light bg-transparent py-4 px-4">
                <div class="d-flex align-items-center">
                    ...
                    <h3 class="fs-2 m-0">Quản Lý Sản Phẩm</h3>
                </div>
            ...
            <div class="container-fluid px-4">
                ...
                <div    class="row my-5">
                    <ul class="quanly">
                    <li><a href="themsanpham.php" class="btn__add" style="padding: 15px 50px; background-color: #f18a1c; border-radius: 4px; text-align: center;padding-top: 8px; color: #fff; font-size: 18px; font-weight: 600;">
                            Thêm mới
                        </a></li>
                    <?php
                if(isset($total_prd)){
            ...
            ?>
                    ...
                    <div class="col">
                        <table class="table bg-white rounded shadow-sm  table-hover">
                            ...
                            <?php
                                include '../config.php';
                                $loaihang = $_GET['id'];
                                $result=mysqli_query($conn, "SELECT * FROM `sanpham` where product_type = $loaihang");
                                $row = mysqli_fetch_assoc($result);
                                $totalRecords = mysqli_query($conn, "SELECT * FROM `sanpham` where product_type = $loaihang");
                                ...
                                $result = mysqli_query($conn, "SELECT * FROM sanpham where product_type = $loaihang LIMIT $start, $limit");
                                while ($row = mysqli_fetch_array($result)){
                            ?>
                                <tr>
                                    <th scope="row"><?php echo $i++ ?></th>
                                    <td><?php echo $row['product_name']?></td>
                                    <td><?= number_format($row['product_cost'], 0, ",", ".") ?></td>
                                    <td><?php echo $row['soluong']?></td>
                                    <td><img src="../img/sanpham/<?php echo $row["product_img"]?>" alt="" style="height: 160px; width: 150px;"></td>
                                    <td>
                                        <a class="btn btn-warning" style=" line-height: 40px; padding: 0px 20px; background-color:#ffc107; border-radius: 3px" href="sua.php?id=<?php echo $row['id']?>">Sửa</a>
                                        <a class="btn btn-danger" style="line-height: 40px; padding: 0px 20px; background-color:#dc3545; border-radius: 0.25rem;" href="xoasanpham.php?id=<?php echo $row['id']?>">Xóa</a>
                                    </td>       
                                </tr>
                            <?php }?>
                            </tbody>
                ...
                <?php for ($i = 1; $i <= $total_page; $i++){
                    ...
                    if ($i == $current_page){
                        ...
                        echo '<li class="pag-item"><a href = "quanlytungloai.php?page='.$i.'">'.$i.'</a></li>';
                    }
                    }?>
                    </ul>
                    </div>
